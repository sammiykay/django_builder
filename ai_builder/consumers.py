...
class ProjectGenerationConsumer(AsyncWebsocketConsumer):
    ...
    async def stream_project_generation(self, project, user_prompt: str):
        ...
        try:
            ...
            # Generate project (run in thread to avoid blocking)
            result = await asyncio.get_event_loop().run_in_executor(
                None, 
                generator.generate_project_from_prompt,
                user_prompt,
                str(project.id)
            )
...
class AIStreamingConsumer(AsyncWebsocketConsumer):
    ...
    async def stream_file_generation(self, project, requirements):
        """Stream file generation with real-time updates"""
        try:
            await self.send_message({
                'type': 'status_update',
            ...
            for filename in files_to_generate:
                ...
                # Generate file content (this would be your actual AI generation)
                file_content = f"# Generated by AI for {project.name}\n# File: {filename}\n\n# Content would be generated based on requirements:\n# {requirements}\n"
                ...
                await self.send_message({
                    'type': 'status_update',
                    'message': f'Created {filename} ({files_count}/{len(files_to_generate)})'
                })
